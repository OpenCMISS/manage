    ...
    if(NOT DEFINED OPENCMISS_MPI)
        ...
        if (MPI_FOUND)
            ...
            if (UNIX AND NOT APPLE)
                ...
                elseif (LINUX_DISTRIBUTION STREQUAL "Fedora" OR LINUX_DISTRIBUTION STREQUAL "RedHat" OR LINUX_DISTRIBUTION STREQUAL "RedHatEnterpriseServer" OR LINUX_DISTRIBUTION STREQUAL "CentOS")
                    set(SUGGESTED_MPI mpich)
                else ()
                    set(SUGGESTED_MPI mpich)
                    log("Linux distribution not recognised setting suggested MPI to ''${SUGGESTED_MPI}.")
                endif ()
                if (SUGGESTED_MPI)
                    log("No MPI preferences given. We suggest '${SUGGESTED_MPI}' on Linux/${LINUX_DISTRIBUTION}")
                else()
                    log("Unknown distribution '${LINUX_DISTRIBUTION}': No default MPI recommendation implemented. Using '${_DEFUALT_MPI}'" WARNING)
                    set(SUGGESTED_MPI ${_DEFUALT_MPI})
                endif()
            elseif(APPLE)
                set(SUGGESTED_MPI mpich) # Previously this was openmpi but this doesn't work well when used through Python bindings.
            elseif(WIN32)
                ...
                log("No default MPI suggestion implemented for your platform. Using '${_DEFAULT_MPI}'" WARNING)
                set(SUGGESTED_MPI ${_DEFAULT_MPI})
            endif()
